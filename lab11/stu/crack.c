...
int main(int argc, char* argv[])
{
    ...
    // TODO: Create the threads. Use NTHREADS as a total.
    // Use the statics[] array as an argument. so each thread knows its number.
    for(int i = 0; i < NTHREADS; i++)
    {
      // try &statics[i] if this doesn't work
      pthread_create(&cracker_threads[i], NULL, crack_hashes, (void*)&statics[i]);
    }    
